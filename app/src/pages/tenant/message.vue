...
import messagePC from '~/components/templates/tenant_pc/message';
import messageSP from '~/components/templates/tenant_sp/message';
...
export default {
  ...
  mounted() {
    ...
    // クエリで指定されたtalkRoomIdが存在しないときはエラーにする
    const roomIdInQuery = Number(this.$route.query.roomId);
    if (roomIdInQuery) {
      if (this.talkRooms.some(talkroom => (talkroom.id === roomIdInQuery))) {
        this.switchTalkRoom({ id: roomIdInQuery });
      } else {
        this.setNotificationMessage('error', 'トークルームが存在しません');
      }
    }
  },
  methods: {
    ...
    async switchTalkRoom(targetTalkRoom) {
      ...
      this.$router.push(`/tenant/message?roomId=${this.currentRoomId}`);
    },
