                ...
                >
                  ...
                  <div class="p-article__buildingInfo">
                    ...
                    <p class="p-article__buildingSub">
                      <i class="p-article__buildingSubIcon fas fa-subway" />{{ building.nearestStation }}徒歩{{ building.timeRequired }}分
                    </p>
                  </div>
                </n-link>
              </li>
        ...
        <ul class="p-article__recommendList">
          ...
          >
            <n-link
              ...
              :to="`/office/building-detail/${article.articleId}`"
            >
              <div
...
export default {
  head() {
    return {
      title: `${this.article.title}｜estie（エスティ）`,
    };
  },
  filters: {
    moment(date) {
  ...
  async asyncData({
    ...
    const article = await app.$axios
      .$get(`/contentful/article/preview/page/${id}`)
      .catch((err) => {
